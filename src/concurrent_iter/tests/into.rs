...
use orx_concurrent_iter::{ConcurrentIter, ConcurrentIterable, IntoConcurrentIter};
...
#[test_matrix([SplitVec::with_doubling_growth_and_fragments_capacity(16), SplitVec::with_linear_growth_and_fragments_capacity(10, 33)])]
fn split_vec_ref_concurrent_iterable<G: Growth>(mut vec: SplitVec<usize, G>) {
    let (nt, n) = (2, 177);
    vec = new_vec(vec, n);
    let rf = &vec;
    let rf2 = &rf;
    let iter = rf2.con_iter();
